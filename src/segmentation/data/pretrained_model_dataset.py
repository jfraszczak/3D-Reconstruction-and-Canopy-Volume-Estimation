...
class PretrainedModelDataset(SemanticSegmentationDataset):
    """
    ...
    semantic segmentation models like Segformer or UPerNet.
    ...
    Attributes:
        ...
        image_processor (transformers.AutoImageProcessor): Object which will be instantiated as one of the image processor used for data processing appropriate for specified pretrained model.

    Methods:
        __len__(): Returns number of images in the dataset.
        __get__(): Returns index-th image and it's segmentation map as torch Tensors.
    """
    def __init__(self, images_path: str, coco_path: str, pretrained_model: str, augment: bool=False) -> None:
        super().__init__(images_path, coco_path, augment)
        self.image_processor = transformers.AutoImageProcessor.from_pretrained(pretrained_model, do_reduce_labels=True, do_center_crop=False, do_rescale=False)
